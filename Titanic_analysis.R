...
options(warn = -1)
train <- read.csv('C:\\Users\\TejYadav\\Desktop\\watson\\train.csv',stringsAsFactors = F)
test <- read.csv('C:\\Users\\TejYadav\\Desktop\\watson\\test.csv',stringsAsFactors = F)
...
age <- full$Age
n = length(age)
# replace missing value with a random samplt from raw data
...
# create histogram  Cabin v.s. Survival
...
# make a histogram   Fare v.s. Survival
...
# make histogram   Embarked v.s. Survival
